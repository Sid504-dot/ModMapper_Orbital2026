...
export async function getUserProfile(userID: string) {
    const { data: profileInfo, error: profileInfoError } = await supabase.from('user_profile').select('profile_name, interest, start_matric_year, used_su').eq('user_id', userID).maybeSingle();
    if (profileInfoError) {
...
export async function upsertUserProfile(userID: string, matricYear: number, userInterest: string, name: string, usedSu: number) {
    
    const { data, error } = await supabase
        .from('user_profile')
        .upsert({user_id: userID, start_matric_year: matricYear, interest: userInterest, profile_name: name, used_su: usedSu}, { onConflict: 'user_id' })
        .select();
